...
# 減速機・モータ由来の粘性摩擦を模して引き上げ、現実的な速度域に収める。
# Phase2-M5 (#31): M2時点では2.0としたが、M4でこの値が大きすぎて残差ネットが
# 摩擦の線形ゲインを学習しきれず、PTP制御が持続振動することが判明。
# dataset_vertical.pyの棄却サンプリング(角速度上限)と併用する前提で1.0に緩和。
C_VISCOUS = np.array([1.0, 1.0])
# Phase2-M12/M13 (#43, #45): C_COULOMB=0.02のままだとcoulomb/viscous比が
# 0.02(Phase1は0.4)まで薄まり、グレーボックスがクーロン係数を識別できない
# ことが判明した。C_VISCOUSを下げる方向は棄却サンプリングの採択率が
# 急落し非現実的(0.5でも8.4%)だったため、C_COULOMBをPhase1と同じ比率
# (0.4)になるよう引き上げる。この変更はむしろ棄却サンプリングの採択率を
# 改善する(全体の摩擦が増えカオス的発散が抑えられるため)ことを確認済み。
C_COULOMB = np.array([0.4, 0.4])
V_STRIBECK = 0.03
