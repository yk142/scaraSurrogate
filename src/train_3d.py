"""Phase3: 3D空間マニピュレータ用のNSSモデル学習スクリプト。

学習ループそのものはPhase 1/2と同じレシピ(マルチステップ損失+カリキュラム
学習)を用い、Phase2-M8の教訓通り損失は成分ごとに正規化する。
"""
import numpy as np
import torch
...
from src.dataset_3d import generate_controlled_trajectories
from src.model_3d import AutoregressiveModel3D, StructuredFrictionGrayBoxModel3D, encode_state
...
def train(
    ...
    model_cls: type[AutoregressiveModel3D] = StructuredFrictionGrayBoxModel3D,
) -> AutoregressiveModel3D:
