...
from src.control_vertical import run_ptp_true


def test_ptp_controller_reaches_target_on_true_plant():
    """重力あり真の物理モデルに対し、計算トルク法PTP制御が目標角度に収束すること。"""
    target = np.array([0.5, -0.5])
    initial_state = np.array([-0.3, 0.1, 0.0, 0.0])
...
def test_ptp_controller_reaches_target_from_large_offset():
    """大オフセットICでも収束すること。"""
    target = np.array([0.5, -0.5])
    initial_state = np.array([1.3, -1.1, 0.5, -0.5])
